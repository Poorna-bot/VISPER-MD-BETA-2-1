...
import fs from 'fs'; // Sync functions (existsSync, mkdirSync) walata
import { promises as fsp } from 'fs'; // Async functions (readFile, writeFile) walata
...
const settingsSchema = new mongoose.Schema({
    ...
    LOGO: { type: String, default: "https://mv-visper-full-db.pages.dev/Data/visper_main.jpeg" },
...
async function readJsonFile() {
    try {
        const data = await fsp.readFile(filePath, 'utf8'); // fsp use karanna
...
async function writeJsonFile(data) {
    await fsp.writeFile(filePath, JSON.stringify(data, null, 2)); // fsp use karanna
}
